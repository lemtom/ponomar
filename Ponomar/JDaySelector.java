...
/**************************************************************
 * JDaySelector: A class for the object that does the heavy duty work in
 * JCalendar The purpuse of this class is to select a day.
 * 
 * Instructions: declare a JCalendar object and work from there.
 * 
 * Copyright information: Adapted from JDayChooser: - A bean for choosing a day
 * Copyright (C) 2004 Kai Toedter kai@toedter.com www.toedter.com
 *
 ...
 * 
 * (C) 2006 ALEKSANDR ANDREEV. PERMISSION IS HEREBY GRANTED TO REPRODUCE,
 * MODIFY, AND/OR DISTRIBUTE THIS SOURCE CODE PROVIDED THAT THIS NOTICE REMAINS
 * IN ALL VERSIONS OR DERIVATIVES THEREOF.
 * 
...
class JDaySelector extends JPanel implements ActionListener, KeyListener, FocusListener {
	...
	private final JButton[] tones;
	private JButton selectedDay;
	private int day;
	private JDate today;
	private final JPanel dayPanel;
	private final JPanel tonePanel;
	private final Color sundayForeground;
	private final Color weekdayForeground;
	private final Color decorBackground;
	private Color oldColor;
	private final Color[] commonBackgrounds = new Color[3];
	private final Color dukeBlue;
	...
	private final NumberFormat numFormat;// = NumberFormat.getInstance(new
	// Locale(Text.Phrases.get("Language").toString(),Text.Phrases.get("Country").toString()));
	// (String)Text.Phrases.get("LanguageMenu")
	private final DecimalFormat df;// =(DecimalFormat)numFormat;
...
//			System.out.println(tmpCalendar.toString());
			...
			/**
			 * else { days[i + n + 7].setBackground(commonBackground); }
			 **/
	...
	private void drawDaysGregorian() {
		...
		// ALIGNING THE JULIAN OBSERVATIONS (FEASTS/FASTS) WITH THEIR CORRESPONDING
		// GREGORIAN DATE!
		...
		PCalendar tmpGregorianC = new PCalendar(
				new JDate(tmpJulianC.getYearG(), tmpJulianC.getMonthG(), tmpJulianC.getDayG()), "gregorian",
				analyse.dayInfo);
		int firstDayOfWeek = 0;
		...
		// we need to determine if we need to consider additional cases before and after
		// since the Gregorian year and Julian year may not align.
		feasts.putAll(Paschalion.getFeasts(tmpGregorianC.getYearJ() - 1, analyse.dayInfo));
		feasts.putAll(Paschalion.getFeasts(tmpGregorianC.getYearJ() + 1, analyse.dayInfo));
...
//			System.out.println(tmpCalendar.toString());
	...
	protected void setDay(int d) {
		 ...
		 * tmpCalendar.addMonths(1); tmpCalendar.subtractDays(1);
		 **/
