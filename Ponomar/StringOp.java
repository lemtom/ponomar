...
/*****************************************************************************
 * StringOp.java :: A CLASS TO HANDLE ADDITIONAL STRING OPERATIONS FOR THE
 * PONOMAR PROGRAM(ME)
 * 
 * (C) 2007 ALEKSANDR ANDREEV. ALL RIGHTS RESERVED. SIGNIFICANT PORTIONS (C)
 * 2007 YURI SHARDT. ALL RIGHTS RESERVED.
 * 
 * IMPORTANT: ****do not copy, modify, or distribute this class**** THIS MODULE
 * IS STILL **IN DEVELOPMENT**, AND MAY NOT BE REPRODUCED, MODIFIED, OR
 * REDISTRIBUTED, IN WHOLE OR IN PART, FOR ANY REASON WITHOUT THE EXPRESS
 * CONSENT AND WRITTEN PERMISSION OF THE AUTHORS. THIS CODE WILL BE MADE PUBLIC
 * ONCE IT HAS REACHED A STABLE LEVEL.
 * 
...
// JOINS THE MEMBERS OF AN ARRAY, ANALAGOUS TO PERL'S join FUNCTION
	...
	private static String join(String[] pieces, char sep) {
		return join(pieces, String.valueOf(sep));
	}

	private static String join(String[] pieces, String sep) {
		if (pieces.length == 0)
			return "";
...
// Hashtable MUST BE INITIALIZED AND VALUES MUST BE ENTERED BEFORE CALLING eval(String) [in a constructor, probably]
	 ...
	 * OF OPERATIONS IDEA: RECEIVE AN EXPRESSION CONTAINING VARS, NUMBERS AND
	 * OPERATORS OF THE FORM nday < 4 && (dow == 5 || dow == 6) PARSE THIS
	 * EXPRESSION PRESERVING THE ORDER OF OPERATIONS AND LOOKING UP THE VARIABLES IN
	 * A HASHTABLE RETURNING true OR false IF THE EXPRESSION IS TRUE OR FALSE OR THE
	 * EXPRESSION'S NUMERICAL VALUE IF IT'S A MATHEMATICAL OPERATION e.g. (80 <
	 * Easter) && Easter > 80 || Easter - 49 < 50 && Easter + 49 > 120 || (Easter +
	 * 49) % 8 <= 5
	 * 
	 * (C) 2007 YURI SHARDT. ALL RIGHTS RESERVED.
	 * 
	 * HISTORY: April 2007 - eval 1.0 by Aleksandr Andreev written to handle basic
	 * logical operators June 2007 - rewritten by Yuri Shardt and Aleksandr Andreev
	 * with added functionality July 2007 - eval 2.0, a complete rewrite of the eval
	 * function to handle mathematical operations, unary and binary logic operators
	 * and correct OOO.
	 ******************************************************************************/

    double eval(String expression) throws IllegalArgumentException {
		...
		// FOR THIS FUNCTION ONLY CASES 2 (SOME AND NOT ALL FUNCTIONS), 3, 4, 6, 7, 11,
		// AND 12 ARE IMPLEMENTED OR EVEN REQUIRED
		...
		// FOR ANY COMBINATION OF SYMBOLS WITH THE SAME PRECEDENCE WITH LEFT TO RIGHT
		// EQUAL PRECEDENCE
		// GOING RIGHT TO LEFT, DETERMINE THE FIRST OCCURANCE OF THE SYMBOL
		// SPLIT AT THIS OCCURRANCE OF THE SYMBOL INTO 2 PARTS: RHS AND RHS.
		// CHECK, IF NECESSARY, FOR ANY UNARY OPERATORS (- (AS IN -100), !), WHICH DO
		// NOT TAKE A SPACE BETWEEN THEM AND THE COMPONENT
		...
		while (test) {
			...
			// SINCE UNARY OPERATORS ARE NOT FOLLOWED BY A SPACE: TEST FOR THE PRESENCE OF A
			// FOLLOWING SPACE
			if (expression.charAt(j + 1) == ' ') {
				test = false;
			} else {
				// A UNARY OPERATOR WAS FOUND; TRY AGAIN STARTING AT j-1: THERE COULD BE ROW OF
				// UNARY "-"
				len = j - 1;
			}
			// TEST IF THE END OF THE STRING HAS BEEN REACHED OR A SINGLE INITIAL UNARY
			// OPERATOR HAS BEEN FOUND ("-900")
			if (j < 1) {
				j = -1;
				test = false;
			}
		...
		// UNLIKE FOR THE OTHER OPERATORS ABOVE, WHICH HAVE LEFT TO RIGHT PRECEDENCE AT
		// EQUAL LEVEL
	...
	boolean evalbool(String expression) {
		// THIS FUNCTION SIMPLY CONVERTS A DOUBLE INTO A BOOLEAN EXPRESSION, WHERE 0 =
		// FALSE
		// EVERYTHING ELSE EQUALS TRUE! THIS FUNCTION IS REQUIRED FOR &&, ||, and !
		// OPERATORS
		...
		// CONVERT TO BOOLEAN
		boolean resultboolean = result != 0;
...
// INTERNAL FUNCTION TO CAST BOOLEAN AS DOUBLE
	...
	public StringOp clone() {
		...
		LinkedHashMap<Object, Object> testing = new LinkedHashMap<>(this.dayInfo);
...
// CAN BE USED TO TEST eval(String) AGAINST OUTPUT FROM KNOWN SOURCE, E.G. MATLAB
	public static void main(String[] argz) {
		...
		/*
		 * dayInfo = new LinkedHashMap<Object, Object>(); dayInfo.put("nday", 3);
		 * dayInfo.put("wday", 6); dayInfo.put("ndayP",81);
		 */
		/*
		 * LinkedHashMap<Object, Object> tested=dayInfo.clone();
		...
		String arg =
				// "(Easter + 5) < (100 +5"; //should fail with an error message stating no
				// closing brackets.
				// "Today + 5 > 100"; //should fail and return an error message with
				// non-existant variable
				// "(Easter + 50 - 100 - 1000) < 500"; //true according to MATLAB
				// "(80 < Easter < 100) || Easter > 80 && Easter - 49 < 50 || Easter + 49 >
				// 120"; //true according to MATLAB
				// "(80 < Easter < 100) && Easter > 80 || Easter - 49 < 50 && Easter + 49 >
				// 120"; //true according to MATLAB
		...
		// "2 + -----Easter"; // -79 |
		// "(Easter + 49) % 8"; // 2 |
		// THE FOLLOWING TESTS WERE ADDED BY A. ANDREEV 8/1/07 N.S. TO CHECK BOOLEAN
		// FUNCTIONALITY |
		// "!true"; // false |
		// "!false"; // true |
		// "true || false"; // true |
		...
		// "60 < Easter < 80"; // true!!!!!! \|/
		/**
		 * v 80 < Easter < 100 is "true" since the function first evaluates 80 < Easter
		 * to be 1, which is < 100. Analogously, 60 < Easter < 80 is evaluated true,
		 * when in fact it should be false.
		 * 
		 * In reality x < y < z is nice functionality, but it does not appear to be
		 * supported natively by e.g. Perl's eval() function. That is too bad, but there
		 * isn't much we can do about it. I suggest reverting back to 80 < Easter &&
		 * Easter < 100 for the sake of cross-language compatibility.
		 * 
