...
class PCalendar implements Cloneable {
	...
	private double difference; // Stores the associated difference in days between Julian and Gregorian
								// calendars
	private final String type; // Stores which calendar I am dealing with. At present only Gregorian and
							// Julian.
	private final JDate date; // Stores the date associated with the given calendar.
	...
	protected PCalendar(JDate date1, String calendar, Map<Object, Object> dayInfo) {
		date = date1;
		type = calendar;
	...
	public int getAM() {
		...
		PCalendar cutoff = new PCalendar(new JDate(9, 1, date.getYear()), julian, new LinkedHashMap<>(analyse.dayInfo));
	...
	public Object clone() {
		return new PCalendar(date, type, new LinkedHashMap<>(analyse.dayInfo));
	}
