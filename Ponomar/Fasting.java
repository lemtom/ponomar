...
public class Fasting implements DocHandler {
	...
	public Fasting(Map<Object, Object> dayInfo) {
	...
	public String FastRules() // throws IOException
	{
		...
		// THIS IS A KLUTZ THAT WILL BE REMOVED ONCE THERE IS A PROPER ABILITY TO RANK
		// THE DAY
		// RANK 1 HOLIDAYS
		// I have removed the klutz, as dRank has been properly implemented! Y.S
		// 2010/02/02 n.s.
		// StringOp.dayInfo.put("dRank",1); //ANY RANK LESS THAN 4 WILL DO
		/*
		 * try { FileReader frf = new
		 * FileReader("Ponomar/xml/Commands/DivineLiturgy.xml"); QDParser.parse(this,
		 * frf); int doy=Integer.parseInt(StringOp.dayInfo.get("doy").toString()); int
		 * nday=Integer.parseInt(StringOp.dayInfo.get("nday").toString()); if(doy == 256
		 * || doy == 358 || doy == 5 || nday == 40 || nday == 50 || doy == 217) {
		 * //THESE ARE CLASS 1 HOLIDAYS StringOp.dayInfo.put("dRank",6); } if ((doy ==
		 ...
		 * WHETHER IT IS 3 OR 4 AS THE RANK. } } } //ADDING SOME FURTHER DAYS THAT ARE
		 * NOT INCLUDED ABOVE if(doy == 6) { StringOp.dayInfo.put("dRank",3); } if(doy
		 * == 127 || doy == 128 || doy == 161 || doy == 169 || doy == 185 || doy == 190
		 * || doy == 199 || doy == 200 || doy == 220 || doy == 227 || doy == 239 || doy
		 * == 241 || doy == 242 || doy == 267 || doy == 281 || doy == 282 || doy == 290
		 * || doy == 291 || doy == 295 || doy == 300 || doy == 328 || doy == 342 || doy
		 * == 345 || doy == 346 || doy == 350 || doy == 23 || doy == 26 || doy == 16 ||
		 * doy == 19 || doy == 8 || doy == 10 || doy == 11 || doy == 13 || doy == 24) {
		 * StringOp.dayInfo.put("dRank",5); }
		 * 
		 * } catch (Exception e1) { e1.printStackTrace(); } //FINISHED THE KLUTZ
		 */
		// ACTUAL PROGRAMME
	...
	public String convert(String fast) {
		/*
		 * COMMON REGULATIONS 0000000 : No Food 0000001 : Strict Fast (Dry Food Only)
		 * 0000011 : Food without oil 0000111 : Food with oil/Fast: Wine and oil allowed
		 * 0001111 : Caviar is permitted 0011111 : Fish is permitted/Fast: Fish, wine,
		 * and oil allowed 0111111 : Meat is excluded 1111111 : No fast 0000010 : Wine
		 * Permitted
		 */

		String[] fastNames = phrases.obtainValues(phrases.Phrases.get("Fasts"));
		...
		String[] item = new String[] { fastNames[13], fastNames[14], fastNames[15], fastNames[16], fastNames[17],
				fastNames[18], fastNames[19] };
	...
	public void startElement(String elem, HashMap table) {
		...
		// DAYS
		/*
		 * if (elem.equals("COMMAND")) { //THIS WILL STORE ALL THE POSSIBLE COMMANDS FOR
