...
class Astronomy {
	...
	public static double[] sunpos(double d) {
		...
		double Eccentric_anomaly = Mean_anomaly_of_sun + Eccentricity_of_Earth_orbit * RADEG * sind(Mean_anomaly_of_sun)
	...
	public double lunarlong(long day) {
		// Computes the apparant lunar longitude based on the method in Meeus, pp.
		// 337-343
		// day must be in the Gregorian calendar
		// Formula does not work far into the future or past due to issues with
		// dynamical time
		// System.out.println("day = " +daysSinceJan0(day));
		double days = daysSinceJan0(day);
		double T = days / 36525;
		// System.out.println(T);
		double Lprime = revolution(
		...
		double additive = 6288774 * sind(Mprime) + 1274027 * E * sind(2 * D - Mprime) + 658314 * sind(2 * D)
				+ 213618 * sind(2 * Mprime);
	...
	public double solarlong(long day) {
		...
		double c = (1.914602 - 0.004817 * t - 0.000014 * t * t) * sind(m) + (0.019993 - 0.000101 * t) * sind(2 * m)
				+ 0.000289 * sind(3 * m);
	...
	public double lunarage(long day) {
		// System.out.println("Lunar Age = " +
		// revolution(lunarlong(day)-solarlong(day)));
		return revolution(lunarlong(day) - solarlong(day));

	}

	public String lunarphase(long day, Map<Object, Object> dayInfo) {
		/*
		 * Determine the lunar phase given various for the given JDE day; Assumes that
		 * the given day is midnight local time.
		 */
	...
	public String lunarphaseJulian(JDate day) {
		/*
		 * Determine the lunar phase for the Julian calendar; Assumes that the given day
		 * is midnight local time.
		 */
		String phase = "Error";
		/*
		 * int Golden=(int) year%30+1; int epact=11*(Golden-1)%30; //The epact gives the
		 * lunar age on March 22. Therefore to determine the //age on other days,
		 * requires us to determine the number of days to March 22 //and then divide by
		 * an appropriate factor 29.5 and take the integer component.
		 * 
		 * int difference=(day-JDate(day.getYear(),3,22)); int epactV=epact+difference;
		 * double todaysAge=epactV-29.5*Math.floor(epactV/29.5); //Not strictly correct,
		 * but will do right now. double tomorrowsAge=35;
		 * 
		 * 
		 * LanguagePack Text = new LanguagePack(); String[] PhaseNames =
		 * Text.obtainValues((String) Text.Phrases.get("Phases")); //Key="Phases"
		 * Value="New Moon/,Waxing Crescent/,First Quarter/,Waxing Gibbous/,Full Moon/,Waning Gibbous/,Third Quarter/,Waning Crescent"
		 * Comment="Lunar Phases" /> if (todaysAge>tomorrowsAge){ //This can only
		 * signify a new moon phase=PhaseNames[0]; } else if (todaysAge>0 &&
		 * tomorrowsAge<90){ //New Moon was yesterday phase=PhaseNames[1];
		 * //System.out.println(todaysAge+" "+tomorrowsAge); } else if (todaysAge <= 90
		 * && tomorrowsAge > 90){ //First Quarter occurred today phase=PhaseNames[2];}
		 * else if (todaysAge > 90 && tomorrowsAge < 180){ //Between first quarter and
		 * full moon phase=PhaseNames[3];} else if(todaysAge <=180 && tomorrowsAge >
		 * 180){ //Full moon phase=PhaseNames[4];} else if (todaysAge > 180 &&
		 * tomorrowsAge<275){ //Between Full moon and last quarter phase=PhaseNames[5];}
		 * else if (todaysAge<=275 && tomorrowsAge>275){ //Last Quarter
		 * phase=PhaseNames[6]; } else if (todaysAge>275 && tomorrowsAge>todaysAge){
		 * //Between Last Quarter and new moon phase=PhaseNames[7];
		 * //System.out.println(todaysAge+" "+tomorrowsAge); }
		 */
