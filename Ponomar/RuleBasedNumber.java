...
/***************************************************************
 * RULEBASEDNUMBER.java :: MODULE THAT CONVERTS A DECIMAL NUMBER TO AN
 * IDEOGRAPHIC NUMBER, THAT IS, 145 -> CXLV
 * 
 * RuleBasedNumber.java is part of the Ponomar project. Copyright 2010 Yuri
 * Shardt version 1.0: May 2010 yuri (dot) shardt (at) gmail.com
 * 
...
class RuleBasedNumber implements DocHandler {
	// {'a',N} means that a will be repeated for each time that the number exceeds
	// by N (less the base amount)
	// $ Represents the rest of the number that is to be converted.
	// # Represents the remainder of a number that is to be converted based on a
	// part, that is,
	// # for 10,000 and a base of 1,000 would represent 10 converted into the local
	// representation.
	// For Roman Numberals
	// private String[]
	// DF={"I{'I',1}","IV","V{'I',1}","IX","X{'X',10}$","XL$","L{'X',10}$","XC$","C{'C',100}$","CD$","D{'C',100}$","CM$","M{'M',1000}$"};
	// private long[] BaN={1,4,5,9,10,40,50,90,100,400,500,900,1000};
	// For Chinese Numberals
	private String[] dF = { "\u4E00", "\u4E8C", "\u4E09", "\u56DB", "\u4E94", "\u516D", "\u4E03", "\u516B", "\u4E5D",
			"#\u5341$", "#\u767E['\u3007(0)','$ < 10']['\u4E00(1)','$ < 20 && $ >= 10']$", "#\u5343$", "#\u842C$",
			"#\u5104$" };
	...
	/*
	 * The following marks are permitted: 1) anything before or after the three
	 * octothorps 2) The three octothorps are delimiters: a) between the first two
	 * octothorps all characters that are to be placed after a character and
	 * counting from the start of the number b) between the last two octothorps all
	 * characters that are to be placed before a character and counting from the end
	 * of a number c) #{'character','location'}## The curly brackets represent a
	 * character that is to be always placed. If there are fewer characters then
	 * location, then the character will go either at the end (as in the example) or
	 * at the start of the number 'location' can be a relative location that
	 * includes any of length(A) to represent the length of the string N the decimal
	 * equivalent of the number d) #['character','location','condition']## The
	 * square brackets represent the conditional format the will only be placed if
	 * the 'condition' is true. same as for location marks can be placed in
	 * condition. 3) Square brackets enclose conditional formatting in the format
	 * ['character','command']. Command will be evaluated using StringOp and the
	 * following are acceptable variables: a) length(A) or length($) the length of
	 * the completed number string (A) or the remaining part ($) or the partial part
	 * (#). b) N will represent the original number For example,
	 * ".#['\u0483','length(A) / 2 - 0.5','length(A) % 2 == 1']['\u0483','length(A) / 2 - 1','length(A) % 2 == 0 $$ N >= 20']['\u0483','length(A) / 2 + 1','length(A) % 2 == 0 $$ N < 20']##."
	 * will display a titlo (\u0483) over the middle letter or if there is no middle
	 * letter over the right-hand letter if N < 20; otherwise over the left-hand
	 * letter; dots are placed before and after the number e.g. (Cyrillic numbers):
	 * .BI\u0483. (=12) or .PB\u0483I. (=112). Note that all the formatting applies
	 * in the order in which the character are stored in Unicode (left-to-right),
	 * the display is irrelevant here.
	 */
	// private String fformat=".#{'!','2'}['$@','2','N > 10']#['<','2','N >
	// 1000']{'>','7'}#."; //Any marks that need to be added on top of the final
	// number
	private String fformat = "###";
	private boolean readRules = false;
	private final StringOp analyse = new StringOp();

	public RuleBasedNumber(Map<Object, Object> dayInfo) {
	...
	public String getFormattedNumber(double number) {
		...
		if (firstAdd != null) {
			...
			while (curlF > -1) {
				...
				formattedNumber = formattedNumber.substring(0, dloc + change) + curlSplit[0].substring(1)
						+ formattedNumber.substring(dloc + change);
				change += 1;
			...
			while (squareF > -1) {
				...
				if (analyse.evalbool(squareSplit[2])) {
					...
					formattedNumber = formattedNumber.substring(0, dloc + change) + squareSplit[0].substring(1)
							+ formattedNumber.substring(dloc + change);
					change += 1;
				}
	...
	private String formatNumber(double number) {
		...
		while (squareF > -1) {
			...
			// squareSplit[2]=squareSplit[2].replace("length(A)",
			// Integer.toString(lengthFN));
			// squareSplit[1]=squareSplit[1].replace("length(A)",
			// Integer.toString(lengthFN));
			// squareSplit[2]=squareSplit[2].replace("N",Double.toString(Number));
			squareSplit[1] = squareSplit[1].replace("N", Double.toString(number));
			...
			if (analyse.evalbool(squareSplit[1])) {
				fNumber = new StringBuilder(
						fNumber.substring(0, squareF) + squareSplit[0].substring(1) + fNumber.substring(squareAF + 1));
